    ...
    visit(tree, (node) => {
      if (node.type === "textDirective" || node.type === "leafDirective" || node.type === "containerDirective") {
        ...
        data.hProperties = h(tagName, {
          class: `admonition admonition-${boxInfo.id} panel panel-${boxInfo.id}`,
        }).properties;
        ...
        if (node.attributes.class) {
          ...
          svgData.hProperties = h("i", {
            class: "fa-solid " + node.attributes.class,
          }).properties;
        } else {
          svg = h("svg");
          const svgData = svg.data || (svg.data = {});
        ...
        // Creating the panel heading header
        const panelHeading = h("h3");
        const panelHeadingData = panelHeading.data || (panelHeading.data = {});
        panelHeadingData.hName = "h3";
        panelHeadingData.hProperties = h("h3", {
          class: `panel-title`,
        }).properties;
        panelHeading.children = [svg, title];

        // Creating the panel heading div
        const panelHeadingDiv = h("div");
        const panelHeadingDivData = panelHeadingDiv.data || (panelHeadingDiv.data = {});
        panelHeadingDivData.hName = "div";
        panelHeadingDivData.hProperties = h("div", {
          class: `panel-heading`,
        }).properties;
        panelHeadingDiv.children = [panelHeading];

        // Creating the panel body
        ...
        node.children = [panelHeadingDiv, contentColPaddingWrapper];
      }
    });
  };
}